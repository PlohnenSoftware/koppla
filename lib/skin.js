...
class SymbolSkin {
    ...
    updateDynamicSize(size) {
        (0, assert_1.default)(this.options?.dynamic);
        this.size.x = size.x;
        this.size.y = size.y;
        (0, assert_1.default)(this.svg.rect?.length === 1);
...
class Skin {
    parsed;
    cache = {};
    styleCache = new css_1.StyleCache("kpl");
    static minimumBoxSize = { x: 50, y: 50 };
    async load(skinFile) {
    ...
    dynamicSymbol(symbolInfo) {
        const boxSize = { ...Skin.minimumBoxSize };
        const box = {
            "@attrs": {},
            rect: [
                {
...
function findTerminalPaths(start) {
    const paths = (start.path ?? []).filter((path) => path["@attrs"]["koppla:terminal"] !== undefined);
    const childPaths = (start.g ?? []).flatMap((node) => {
